...
class AgoraVideoAuth {
  ...
  final String currentUserToken; // customer token
  final String astroToken;       // astrologer token
  final String currentUserId;    // <-- needed for userAccount on the customer app
  final String astroId;          // <-- handy for asserts / logs
  ...
  factory AgoraVideoAuth.fromJson(Map<String, dynamic> j) {
    return AgoraVideoAuth(
      ...
      currentUserId:    j['current_user_id'] ?? '',  // <-- your API returns this
      astroId:          j['astro_id'] ?? '',         // <-- your API returns this
    );
  }
}
